...
def get_dataloader(dataset, shuffle, batch_size = 16, n_workers = 0): 
    # changed n_workers from n_workers = multiprocessing.cpu_count() to n_workers = 0
...
def load_model(net, fname, dir_out = '../out/'):
    ...
    net.load_state_dict(torch.load(dir_out+fname, map_location={'cuda:0': 'cpu'})) # added map_location={'cuda:0': 'cpu'}
...
def load_state_dict(fname, dir_in ='../out/'):
    ...
    return torch.load(dir_in + fname, map_location={'cuda:0': 'cpu'}) # added map_location={'cuda:0': 'cpu'}
